...
impl GuiParts for Gui {
    ...
    fn cre_bord(&mut self) {
        self.value[0] = [true; 18];
        self.value[17] = [true; 18];
        for num in 0..18 {
            self.value[num][0] = true;
            self.value[num][17] = true;
        }
    }
    ...
    fn toggle_theme(&mut self) {
        ...
        let mut commds = [process::Command::new(rm()), process::Command::new("mkdir")];
        if !windows() {
            commds[0].arg("-r");
        }
...
fn home() -> String {
    ...
    match platform {
        Platform::Windows => format!("C:/Users/{}/", username),
...
fn rm() -> String {
    let platform = whoami::platform();
    match platform {
        Platform::Windows => "rmdir".to_string(),
        Platform::Linux | Platform::MacOS => "rm".to_string(),
        _ => {
            process::exit(1);
        }
    }
